...
//先隐藏手势和控制栏，防止在没有prepare的时候做操作。
    ...
    private void onWifiTo4G() {
        ...
        if (mAliyunLocalSource != null) {
            String path = mAliyunLocalSource.getSource();
    ...
    private void initControlView() {
        ...
        mControlView.setOnMenuClickListener(new ControlView.OnMenuClickListener() {
            ...
            public void onMenuClick() {
                //点击之后显示倍速界面
                mControlView.setMenuStatus(false);//隐藏菜单按钮
                mSpeedView.show(mCurrentScreenMode); //根据屏幕模式，显示倍速界面
            }
        });
        //清晰度按钮点击
    ...
    private boolean generateMediaInfo() {
        ...
        if (mAliyunMediaInfo == null) {
            ...
            int newDuration = (int) mAliyunVodPlayer.getDuration();
            changed = (changed || (oldDuration != newDuration));
            mAliyunMediaInfo.setDuration(newDuration);
            //使用用户设置的标题
            String oldTitle = mAliyunMediaInfo.getTitle();
            String newTitle = getTitle(mAliyunMediaInfo.getTitle());
            changed = changed || (!TextUtils.equals(newTitle, oldTitle));
            mAliyunMediaInfo.setTitle(newTitle);

            String oldPost = mAliyunMediaInfo.getPostUrl();
            String newPost = getPostUrl(mAliyunMediaInfo.getPostUrl());
            changed = changed || (!TextUtils.equals(newPost, oldPost));
     ...
     * @param errorMsg   错误描述
     */
    private void showErrorTipView(int errorCode, int errorEvent, String errorMsg) {
        ...
        if (context instanceof Activity) {
            if (finalScreenMode == AliyunScreenMode.Full) {
                ...
                if (getLockPortraitMode() == null) {
                    ...
                    //如果是固定全屏，那么直接设置view的布局，宽高
                    ViewGroup.LayoutParams aliVcVideoViewLayoutParams = getLayoutParams();
                    aliVcVideoViewLayoutParams.height = (int) (ScreenUtils.getWidth((Activity) context) * 9.0f / 16);
                    aliVcVideoViewLayoutParams.width = ViewGroup.LayoutParams.MATCH_PARENT;
                }
            }
        }
    ...
    private void handleProgressUpdateMessage(Message msg) {
        if (mAliyunVodPlayer != null && !inSeek) {
            ...
            int currentPosition = (int) mAliyunVodPlayer.getCurrentPosition();
            VcPlayerLog.e("lfj0416" , " currentPosition = " + currentPosition + " , duration = " + mAliyunVodPlayer.getDuration());
