...
public class HomeAdapter extends BaseMultiItemQuickAdapter<HomeMutiBean, BaseViewHolder> {
    ...
    public HomeAdapter(List data) {
        ...
        /*setMultiTypeDelegate(new MultiTypeDelegate<HomeBean.IssueListBean.ItemListBean>() {
            @Override
            protected int getItemType(HomeBean.IssueListBean.ItemListBean itemListBean) {
                return Integer.parseInt(itemListBean.getType());
            }
        });
        getMultiTypeDelegate().registerItemType().registerItemType();*/
    }
