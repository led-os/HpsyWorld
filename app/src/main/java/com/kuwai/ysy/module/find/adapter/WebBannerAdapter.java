...
public class WebBannerAdapter extends RecyclerView.Adapter<WebBannerAdapter.MzViewHolder> {
    ...
    public WebBannerAdapter(Context context, FoundBean foundBean) {
...
//        this.urlList = urlList;
    ...
    public void onBindViewHolder(WebBannerAdapter.MzViewHolder holder, final int position) {
        if (foundBean.getData().getBanner() == null || foundBean.getData().getBanner().isEmpty())
            return;
        final int P = position % foundBean.getData().getBanner().size();
//        String url = ;
        ImageView img = (ImageView) holder.imageView;
        Glide.with(context).load(foundBean.getData().getBanner().get(P).getImg()).into(img);
    ...
    public int getItemCount() {
        if (foundBean.getData().getBanner() != null) {
            return foundBean.getData().getBanner().size();
        }
        return 0;
    }
