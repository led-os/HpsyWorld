    ...
    public void initView(Bundle savedInstanceState) {
        ...
        mDateAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                if (mDateAdapter.getData().get(position).getType() == 2) {
                    //树洞
                    Intent intent = new Intent(getActivity(), HoleDetailActivity.class);
                    intent.putExtra("tid", String.valueOf(mDateAdapter.getData().get(position).getT_id()));
                    startActivity(intent);
                } else {
                    //动态
                    Intent intent = new Intent(getActivity(), DyDetailActivity.class);
                    Bundle bundle = new Bundle();
                    bundle.putString("did", String.valueOf(mDateAdapter.getData().get(position).getT_id()));
