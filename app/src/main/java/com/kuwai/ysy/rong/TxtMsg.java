...
@MessageTag(value = "TxtMsg", flag = MessageTag.ISCOUNTED | MessageTag.ISPERSISTED)
public class TxtMsg extends MessageContent {
    ...
    public static TxtMsg obtain(String text) {
        TxtMsg model = new TxtMsg();
        model.setContent(text);
        return model;
    }

    public TxtMsg(byte[] data) {
        String jsonStr = null;
        try {
     ...
     */
    public static final Creator<TxtMsg> CREATOR = new Creator<TxtMsg>() {

        @Override
        public TxtMsg createFromParcel(Parcel source) {
            return new TxtMsg(source);
        }

        @Override
        public TxtMsg[] newArray(int size) {
            return new TxtMsg[size];
        }
    };
