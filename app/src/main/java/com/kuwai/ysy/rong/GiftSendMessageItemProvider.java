...
@ProviderTag(
        messageContent = GiftSendMessage.class,
        showProgress = false,
        showWarning = false
)
public class GiftSendMessageItemProvider extends IContainerItemProvider.MessageProvider<GiftSendMessage> {
    ...
    public View newView(Context context, ViewGroup group) {
        View view = LayoutInflater.from(context).inflate(R.layout.rong_gift, null);
        GiftSendMessageItemProvider.ViewHolder holder = new GiftSendMessageItemProvider.ViewHolder();
        holder.message = (TextView) view.findViewById(R.id.tv_num);
        holder.img = view.findViewById(R.id.img_center);
        view.setTag(holder);
        return view;
    }

    @Override
    public Spannable getContentSummary(GiftSendMessage data) {
        return null;
    }

    @Override
    public Spannable getContentSummary(Context context, GiftSendMessage data) {
        if (data == null)
            return null;
    ...
    @Override
    public void onItemClick(View view, int position, GiftSendMessage content, UIMessage message) {
        /*if (message.getMessageDirection() == Message.MessageDirection.RECEIVE) {
            ...
            intent.putExtra("rid", content.getContent());
    ...
    @Override
    public void onItemLongClick(final View view, int position, final GiftSendMessage content, final UIMessage message) {

        GiftSendMessageItemProvider.ViewHolder holder = (GiftSendMessageItemProvider.ViewHolder) view.getTag();
        ...
        OptionsPopupDialog.newInstance(view.getContext(), items).setOptionsPopupDialogListener(new OptionsPopupDialog.OnOptionsItemClickedListener() {
            ...
            public void onOptionsItemClicked(int which) {
                if (which == 0) {
                    ...
                    clipboard.setText(((GiftSendMessage) content).getContent());
                } else if (which == 1) {
    ...
    @Override
    public void bindView(final View v, int position, final GiftSendMessage content, final UIMessage data) {
        GiftSendMessageItemProvider.ViewHolder holder = (GiftSendMessageItemProvider.ViewHolder) v.getTag();
        ...
        holder.message.setText("x" + content.getExtra());
        GlideUtil.load(v.getContext(), content.getContent(), holder.img);
